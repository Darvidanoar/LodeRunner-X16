...
int loadFiles()
{
    ...
    //result = vload_host("palette.bin",0x11000);
    if (result) 
    ...
    //result = load_bank_host("splash.bin",0x1a000);
    if (result) {
        printf("  loaded splash tilemap - %x\n",result);
        printf("load_bank_host: RAM_BANK=%x\n",RAM_BANK);
    } else {
        printf("  failed to load splash tilemap\n");
        return 0;
    }
    
    printf("pre-exit: RAM_BANK=%x\n",RAM_BANK);
